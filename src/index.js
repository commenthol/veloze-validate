...
export const numberT = (opts) => {
  ...
  const _numberT = (v, e = {}) => {
    ...
    if (exclusiveMin ? v <= min : v < min) {
      e.message = exclusiveMin
        ? `number less equal than min=${min}`
        : `number less than min=${min}`
      return false
    }
    if (exclusiveMax ? v >= max : v > max) {
      e.message = exclusiveMax
        ? `number greater equal than max=${max}`
        : `number greater than max=${max}`
      return false
    }
    if (validate && !validate(v, e)) {
